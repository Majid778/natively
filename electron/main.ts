...
export class AppState {
  ...
  private setupAutoUpdater(): void {
    ...
    // Beta releases are GitHub *prereleases*, so allowPrerelease is required (without
    // it, electron-updater hits /releases/latest, which is stable-only → 404 →
    // "No published versions on GitHub").
    //
    // Do NOT pin channel = 'latest'. Our versions look like 2.2.0-oss-beta.N, whose
    // semver pre-release channel is 'oss-beta'. electron-updater's GitHubProvider only
    // selects a release when its channel matches the updater's channel (or is
    // alpha/beta); forcing 'latest' made 'oss-beta' releases match nothing → still
    // "No published versions". Leaving channel unset derives it from the running
    // version ('oss-beta'), which matches our betas; the manifest lookup then falls
    // back to latest.yml / latest-mac.yml (what electron-builder publishes).
    autoUpdater.allowPrerelease = true
    console.log(`[AutoUpdater] allowPrerelease: ${autoUpdater.allowPrerelease}, channel: ${autoUpdater.channel ?? '(derived from version)'}`)
  ...
  private isVersionNewer(current: string, latest: string): boolean {
    // Parse "MAJOR.MINOR.PATCH[-prerelease]" (e.g. "2.2.0-oss-beta.3"). Splitting on
    // the FIRST '-' only, so a multi-hyphen pre-release tag like "oss-beta.3" stays
    // intact. Earlier code stripped the pre-release entirely, which made every beta
    // compare equal to its base version ("up to date" forever).
    ...
    // Core versions equal — apply SemVer pre-release precedence.
    if (c.pre === l.pre) return false;
    if (!l.pre) return !!c.pre;   // a full release outranks its pre-release
    if (!c.pre) return false;     // current is the full release; pre-release is not newer
    ...
    for (let i = 0; i < Math.max(cp.length, lp.length); i++) {
      const ci = cp[i];
      const li = lp[i];
      if (li === undefined) return false; // current has more identifiers → current is higher
      if (ci === undefined) return true;  // latest has more identifiers → latest is higher
      const cn = Number(ci);
      const ln = Number(li);
      if (!isNaN(cn) && !isNaN(ln)) {
        ...
        return li > ci; // lexical comparison for non-numeric identifiers
      }
    }
    return false;
  }
